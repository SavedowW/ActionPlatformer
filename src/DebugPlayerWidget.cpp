...
void DebugPlayerWidget::draw(Renderer &renderer_, Camera &camera_)
{
    ...
    for (const auto &el : m_pc->getComponent<ComponentObstacleFallthrough>().m_ignoredObstacles)
    ...
    auto &ptransform = m_pc->getComponent<ComponentTransform>();
    auto &pphysical = m_pc->getComponent<ComponentPhysical>();

    m_textManager.renderText("Player pos: " + utils::toString(ptransform.m_pos), 0, {400, 1});
    m_textManager.renderText("Player vel: " + utils::toString(m_pc->accessPreEditVelocity()), 0, {400, 1 + 22 * 1});
    m_textManager.renderText("Player inr: " + utils::toString(pphysical.m_inertia), 0, {400, 1 + 22 * 2});
    ...
    m_textManager.renderText(std::string("On slope: ") + std::to_string(pphysical.m_onSlopeWithAngle), 0, {400, 1 + 22 * 7});
